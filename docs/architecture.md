...
- **Front Office (Real-Time Monitoring, Fraud, and Customer Service)** -- 11 agents:
  - `front_office_router` dispatches into two workflows: `front_office_transaction_workflow` (`transaction_monitoring_agent` -> `fraud_detection_agent` -> `compliance_agent` -> `decision_making_agent`) and `customer_service_chatbot_workflow` (`chatbot_intent_agent` -> `knowledge_base_agent` or `customer_service_agent` -> `transaction_processing_agent`).
  - Tools: `get_customer_context`, `get_transaction_context`, `list_recent_transactions`, `submit_manual_review`, `execute_transfer`, `search_policies`, `search_faqs`, `get_customer_financial_profile`, `search_product_catalog`, `search_loan_playbooks`.
...
- **Mid Office (Operational Planning and Representative Assist)** -- 12 agents:
  ...
  - Tools: `get_operations_snapshot`, `get_support_playbooks`, `persist_validated_schedule`, plus the front-office customer/product/loan tools above (representative assist needs the same customer data).
