...
evaluator_registry.register("artifact_integrity", {"description": "Re-hashes traces.jsonl and confirms run_id consistency with run_manifest.json", "module": "mantis.evaluation.evaluators:TraceEvaluator.evaluate_artifact_integrity"})
evaluator_registry.register("banking_workflow_coverage", {"description": "Reports which banking domains (front/mid/back office) were exercised in the run", "module": "mantis.evaluation.evaluators:TraceEvaluator.evaluate_banking_workflow_coverage"})
evaluator_registry.register("hook_coverage", {"description": "Scores how many of the 10 required control-point hook pairs fired at least once", "module": "mantis.evaluation.evaluators:TraceEvaluator.evaluate_hook_coverage"})
evaluator_registry.register("attack_ground_truth", {"description": "For attack runs, checks whether the configured plugin fired and whether its effect is corroborated by the run's ground truth", "module": "mantis.evaluation.evaluators:TraceEvaluator.evaluate_attack_ground_truth"})
