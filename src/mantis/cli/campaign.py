...
class CampaignManager:
    ...
    async def execute_campaign(self):
        ...
        for idx, config_path in enumerate(configs, 1):
            ...
            # traces/evaluation. Written into run_artifacts/<run_name>/ so
            # the later aggregation copytree below sweeps it into the
            # campaign output alongside everything else for that run; if
            # the run crashed before even creating that directory, create
            # it here so the log survives regardless.
            run_name = self._run_name_for(config_path)
            run_dir = Path("run_artifacts") / run_name
            run_dir.mkdir(parents=True, exist_ok=True)
            (run_dir / "run.log").write_text(
                f"$ mantis --run {config_path}\nexit code: {process.returncode}\n\n"
    ...
    def generate_report(self):
        ...
        # not only a stdout printout that disappears once the terminal
        # scrolls past it.
