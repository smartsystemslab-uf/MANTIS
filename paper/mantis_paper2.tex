...
Table~\ref{tab:extensions} summarizes each extension's verification evidence. All four were checked against real data or a real live run rather than a mock, consistent with the first paper's own testing philosophy; none required a change to \texttt{mantis.core}, \texttt{mantis.hooks}, or the banking workflow modules. The offline suite -- covering all four extensions, including the three additional exporters added in this revision -- stands at 139 passing tests, still executing in well under a minute with no live LLM key required: 4 for the Jaeger exporter and 9 for Grafana/Phoenix/Langfuse together, 6 for the guardrail (including one exercising the real hook bus with both an attack and a guardrail plugin registered, confirming the guardrail sees the attack's already-mutated payload), 7 for the dispute workflow, and 10 for the UI, on top of the base suite the first paper's own work established.
...
We implemented four of the five extensions the first MANTIS paper explicitly deferred, to the same evidentiary standard that paper set for itself: live-verified behavior, checked against the actual emitted trace, with the evidence checked into the repository rather than described in prose. One surfaced a real defect on first implementation -- a scenario-design gap that silently produced zero tool calls across three live trials -- invisible from a clean process exit, found only because that extension's own verification pass insisted on checking real behavior rather than accepting that a run had completed. Revisiting the exporters extension specifically, to replace a single-backend anecdote with broader coverage, found that the remaining three named exporter backends were still unbuilt; all three are now live-verified to the same standard as the first. None of the four required a change to the first paper's core interfaces. The fifth extension, the Zero Trust Backplane, was built and repeated-trial-verified to this same standard -- including an exhaustive check of every (agent, tool) pair a live inventory implies, and a real event-misclassification defect found and fixed along the way -- but is being carried forward as a dedicated integration effort with a real, independently-developed Zero Trust Backplane project for this same testbed~\cite{ztb_repo} rather than reported as part of this paper's own repository and evidence. Future work includes repeated-trial efficacy statistics for the guardrail's own denial path, and continuing that separate Zero Trust integration effort toward a production integration between this testbed's inventory and a real signed decision pipeline.
